...
$customer_ids = [];
if (isset($_GET['id'])) $customer_ids[] = intval($_GET['id']);
if (isset($_GET['ids'])) $customer_ids = array_map('intval', explode(',', $_GET['ids']));
if (isset($_GET['bulk'])) {
...
if (empty($customer_ids)) { die("No subscriber selected"); }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logo-header { width: 100%; max-height: 70px; display: block; margin-bottom: 5px; }
        ...
        .pending-box { border: 1px solid #d32f2f; background: #fff5f5; padding: 4px 8px; font-size: 11px; display: flex; justify-content: space-between; margin-top: 4px; color: #d32f2f; }
...
<body>
    ...
    <?php foreach ($customer_ids as $cid):
        ...
        // Calculate TOTAL pending balance across ALL unpaid billings
        $pending_balance_row = $conn->query("SELECT COALESCE(SUM(b.net_amount),0) - COALESCE((SELECT SUM(p2.amount_paid) FROM payments p2 JOIN billings b2 ON p2.billing_id = b2.billing_id WHERE b2.customer_id = $cid),0) as pending FROM billings b WHERE b.customer_id = $cid")->fetch_assoc();
        $total_pending = max(0, $pending_balance_row['pending']);
        ...
        $inst_fee = 0; $reconnection_fee = 0; $adjustment = 0;
        foreach ($fees as $f) {
    ...
    <div class="statement-page">
        <img src="images/headerlogo.png" alt="Nova Link Digital Systems Corp." class="logo-header">
        ...
        <div class="content-area">
            <div class="bill-summary">
                ...
                <table class="summary-table">
                    <tr><td class="label-col">&nbsp;&nbsp;Inst./Service Fee</td><td class="sep-col">:</td><td class="amount-col"><?php echo $service_fee > 0 ? number_format($service_fee, 2) : '-'; ?></td></tr>
                ...
                <?php if ($total_pending > 0 && $total_pending != max(0, $total_due)): ?>
                <div class="pending-box">
                    <span><strong>Total Pending Balance (all months) :</strong></span>
                    <span><strong><?php echo number_format($total_pending, 2); ?></strong></span>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="reminder-box">
                ...
                foreach ($lines as $line):
                    ...
                    $is_important = (stripos($line, 'DISCONNECTION') !== false);
                ?>
