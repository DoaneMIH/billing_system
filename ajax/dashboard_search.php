...
// Search customers by name, account number, account name
$sql = "SELECT c.customer_id, c.account_number, c.subscriber_name, c.account_name, c.address, c.status, a.area_name 
        ...
        WHERE c.subscriber_name LIKE '%$search%' 
        OR c.account_number LIKE '%$search%' 
        OR c.account_name LIKE '%$search%'
...
while ($row = $r->fetch_assoc()) {
    $sc = match($row['status']) { 'active'=>'success', 'disconnected'=>'danger', 'reconnected'=>'info', default=>'secondary' };
    $results[] = [
        'type' => 'customer',
        'name' => $row['subscriber_name'],
        'detail' => $row['account_number'] . ' | ' . ($row['area_name'] ?? $row['address']),
        'url' => 'customer_ledger.php?id=' . $row['customer_id'],
